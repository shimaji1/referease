...
function AcceptContent() {
  ...
  useEffect(() => {
    ...
    lookupInviteByToken(token).then(setInvite)
  }, [token])

  const finish = async (userId) => {
    const ok = await acceptInvite(token, userId)
    setLoading(false)
        ...
        <div className="w-full max-w-md">
          ...
          {invite && done && (
            <div className="bg-white border-2 border-emerald-300 rounded-xl p-8 text-center">
              ...
              <p className="text-sm text-gray-500 mb-4">You now have access to manage {invite.providers?.name}'s listing.</p>
          ...
          {invite && !done && (
            <div className="animate-fade-in">
              ...
              <p className="text-sm text-gray-500 mt-1 mb-6">You've been given staff access to manage <strong>{invite.providers?.name}</strong>'s listing.</p>
