...
const ALL_SERVICES = [...new Set(PROVIDERS.flatMap(p => p.services))].sort()
const ALL_SPECIALTIES = [...new Set(PROVIDERS.map(p => p.type))].sort()
const ALL_LANGUAGES = [...new Set(PROVIDERS.flatMap(p => p.languages))].sort()
...
function isOpenNow(h) { const day = getDayName(); const s = h[day]; if (!s) return false; const now = getCurrentTime(); const [o,c] = s.split("-"); return c === "24:00" ? now >= o : now >= o && now < c }
function isOpenWeekends(h) { return !!(h.sat || h.sun) }
function isOpenEvenings(h) { return Object.values(h).some(s => { if (!s) return false; return s.split("-")[1] > "18:00" }) }
    ...
    <div style={{ background:"var(--c1)", border:`1px solid ${isFav ? "var(--ac)60" : "var(--bd)"}`, borderRadius:"12px", padding:"14px 16px", position:"relative" }}>
      ...
      <button onClick={() => onSelect(p)} style={{ all:"unset", cursor:"pointer", display:"block", width:"calc(100% - 30px)" }}>
        ...
        <div style={{ display:"flex", flexWrap:"wrap", gap:"6px", marginTop:"8px", alignItems:"center" }}>
          {p.acceptingReferrals ? <Pill color="var(--green)">Accepting</Pill> : <Pill color="var(--red)">Not Accepting</Pill>}
          <WaitBadge weeks={p.waitWeeks} />
        ...
        {p.rating && <div style={{ display:"flex", gap:"8px", alignItems:"center", marginTop:"8px" }}><Stars r={p.rating} /><span style={{ fontSize:"10.5px", color:"var(--t3)" }}>({p.reviews})</span></div>}
      </button>
    </div>
  )
}
...
function Detail({ p, onBack, isFav, onFav }) {
  ...
  const R = ({ l, v }) => <div style={{ display:"flex", justifyContent:"space-between", padding:"3px 0", fontSize:"12px", gap:"10px" }}><span style={{ color:"var(--t3)", flexShrink:0 }}>{l}</span><span style={{ color:"var(--t1)", fontWeight:500, textAlign:"right" }}>{v}</span></div>
  return (
    <div style={{ animation:"fadeIn 0.2s ease" }}>
      ...
      <div style={{ background:"var(--c1)", border:"1px solid var(--bd)", borderRadius:"14px", padding:"20px", marginBottom:"14px" }}>
        ...
        <div style={{ display:"flex", gap:"8px", alignItems:"center", marginTop:"12px", flexWrap:"wrap" }}>
          {p.rating && <><Stars r={p.rating} /><span style={{ fontSize:"11px", color:"var(--t3)" }}>({p.reviews} reviews)</span></>}
          {p.acceptingReferrals ? <Pill color="var(--green)">Accepting Referrals</Pill> : <Pill color="var(--red)">Not Accepting</Pill>}
          <WaitBadge weeks={p.waitWeeks} />
          <Pill color={open ? "var(--green)" : "#6b7280"}>{open ? "Open now" : "Closed"}</Pill>
      ...
      <div style={{ display:"grid", gridTemplateColumns:"repeat(auto-fit, minmax(240px, 1fr))", gap:"10px" }}>
        <B title="Contact & Location">
          ...
          <R l="Languages" v={p.languages.join(", ")} />
        </B>
        <B title="Hours">{DAYS.map((d,i) => <R key={d} l={DAY_LABELS[i].slice(0,3)} v={p.hours[d] || "Closed"} />)}</B>
        <B title="Referral Info">
          <R l="Wait" v={p.waitWeeks === null ? "Varies" : p.waitWeeks === 0 ? "No wait" : `~${p.waitWeeks} week${p.waitWeeks > 1 ? "s" : ""}`} />
          <R l="Requirements" v={p.requirements} />
        </B>
        {p.doctors.length > 0 && <B title="Physicians">{p.doctors.map((d,i) => <div key={i} style={{ padding:"4px 0", fontSize:"12.5px", borderBottom:i<p.doctors.length-1?"1px solid var(--bd)":"none" }}>{d}</div>)}</B>}
        <B title="Services"><div style={{ display:"flex", flexWrap:"wrap", gap:"4px" }}>{p.services.map(s => <span key={s} style={{ padding:"3px 8px", borderRadius:"6px", fontSize:"11px", background:"var(--ac)15", color:"var(--ac)", border:"1px solid var(--ac)25" }}>{s}</span>)}</div></B>
      </div>
    </div>
  )
}
...
export default function ReferEaseApp() {
  ...
  const filtered = useMemo(() => {
    ...
    if (acc) r = r.filter(p => p.acceptingReferrals)
    if (on) r = r.filter(p => isOpenNow(p.hours))
    if (we) r = r.filter(p => isOpenWeekends(p.hours))
    if (ev) r = r.filter(p => isOpenEvenings(p.hours))
    if (mw) r = r.filter(p => p.waitWeeks !== null && p.waitWeeks <= parseInt(mw))
    if (mr) r = r.filter(p => p.rating && p.rating >= parseFloat(mr))
    ...
    if (sort==="wait") r=[...r].sort((a,b)=>(a.waitWeeks??999)-(b.waitWeeks??999))
    ...
    return r
  }, [search,cat,spec,svc,lang,acc,on,we,ev,mw,mr,md,sort,showFavs,favs])
        ...
        {view === "detail" && sel ? <Detail p={sel} onBack={() => setView("search")} isFav={favs.includes(sel.id)} onFav={toggleFav} /> : (
          <>
            ...
            {showF && (
              <div style={{ background:"var(--c1)", border:"1px solid var(--bd)", borderRadius:"10px", padding:"14px 16px", marginBottom:"14px", animation:"fadeIn 0.15s ease" }}>
                <div style={{ display:"flex", flexWrap:"wrap", gap:"8px", marginBottom:"10px" }}>
                  <select value={spec} onChange={e => setSpec(e.target.value)} style={sel_s}><option value="">All specialties</option>{ALL_SPECIALTIES.map(s => <option key={s} value={s}>{s}</option>)}</select>
                  <select value={svc} onChange={e => setSvc(e.target.value)} style={sel_s}><option value="">All services</option>{ALL_SERVICES.map(s => <option key={s} value={s}>{s}</option>)}</select>
                  <select value={lang} onChange={e => setLang(e.target.value)} style={sel_s}><option value="">Any language</option>{ALL_LANGUAGES.map(l => <option key={l} value={l}>{l}</option>)}</select>
                </div>
                <div style={{ display:"flex", flexWrap:"wrap", gap:"8px", marginBottom:"10px" }}>
            ...
            <div style={{ marginTop:"28px", padding:"16px", textAlign:"center", fontSize:"11px", color:"var(--t3)", lineHeight:1.6, borderTop:"1px solid var(--bd)" }}>
              <strong style={{ color:"var(--t1)" }}>ReferEase</strong> — {PROVIDERS.length} providers · Thornhill & GTA, Ontario · Prototype
            </div>
          </>
        )}
      </div>
